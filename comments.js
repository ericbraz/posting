...
function commentsManager(comments) {
    ...
    comments.forEach(comment => {
        ...
        params[0].innerText = comment['name'].replace(['.', '_'], ' ');
...
const extractData = async () => {
    ...
    const totalComments = document.getElementById('interactions').getElementsByTagName('p');
    totalComments[2].innerText = `${comments.length} comments`;
}
